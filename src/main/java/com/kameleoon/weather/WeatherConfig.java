 ...
 * Use {@link Builder} to customize caching and API behavior.
 */
public final class WeatherConfig {
    ...
    public static class Builder {
        ...
        public Builder cacheSize(int size) {
            if (size <= 0) throw new IllegalArgumentException("cacheSize must be positive");
            this.cacheSize = size;
            return this;
        }

        public Builder cacheTtlSeconds(long seconds) {
            if (seconds <= 0) throw new IllegalArgumentException("cacheTtlSeconds must be positive");
